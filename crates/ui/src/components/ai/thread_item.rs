    ...
    fn render(self, _: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let title_label = if let Some(title_slot) = self.title_slot {
            ...
            Label::new(title)
                ...
                .when(!opaque_window || self.hovered || self.compact, |label| {
                    label.truncate()
                })
                .into_any_element()
        } else {
        ...
        v_flex()
            ...
            .child(
                h_flex()
                    ...
                    // The gradient only works on opaque windows.
                    .when(
                                ...
                                this.when_some(self.action_slot, |this, slot| {
                                    this.child(
                                        h_flex()
                                            ...
                                            // truncates, so it isn't needed. It
                                            // also only works on opaque windows.
