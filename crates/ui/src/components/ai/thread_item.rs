    ...
    fn render(self, _: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let title_label = if let Some(title_slot) = self.title_slot {
            ...
            Label::new(title)
                ...
                .when(self.hovered || self.compact || !opaque_window, |label| {
                    label.truncate()
                })
                .into_any_element()
        } else {
        ...
        v_flex()
            ...
            .child(
                h_flex()
                    ...
                    // The gradient also only applies in opaque windows; in a
                    // transparent window the title is truncated instead.
                    .when(
                        self.is_truncated && opaque_window && !self.hovered && !compact,
                                ...
                                this.when_some(self.action_slot, |this, slot| {
                                    this.child(
                                        h_flex()
                                            ...
                                            // truncates, and in transparent
                                            // windows the gradient isn't drawn.
                                            .when(!compact && opaque_window, |this| {
                                                this.child(
